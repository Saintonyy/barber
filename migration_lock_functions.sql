...
-- Returns: lock record or NULL if slot is already locked/booked
-- ============================================================================
...
BEGIN
  -- Compute deterministic advisory lock key from barber+date+start
  ...
  -- Check for existing active lock on overlapping time range
  ...
  -- Check for existing confirmed appointment overlapping this slot
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND TIME(scheduled_at) < p_end_time
    AND (TIME(scheduled_at) + (duration_minutes * INTERVAL '1 minute'))::TIME > p_start_time
  LIMIT 1;
...
-- Extends TTL for active users still in booking flow
-- ============================================================================
...
-- ============================================================================
-- GET AVAILABLE SLOTS
-- Returns time slots for a barber on a date, marking locked/booked ones
-- ============================================================================
...
BEGIN
  ...
  WHILE v_current_slot < v_work_end LOOP
    ...
    LEFT JOIN appointments a ON
      ...
      AND TIME(a.scheduled_at) < v_slot_end
      AND (TIME(a.scheduled_at) + (a.duration_minutes * INTERVAL '1 minute'))::TIME > v_current_slot
...
-- Returns all conflicts for a barber on a date
-- ============================================================================
...
BEGIN
  ...
  WHERE a1.tenant_id = p_tenant_id
    ...
    AND DATE(a1.scheduled_at) = p_date;
END;
$$;
